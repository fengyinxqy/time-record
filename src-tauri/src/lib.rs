...
fn rename_project(
    ...
    database(&state)?.rename_project(project_id, &new_name, now_seconds())
}
...
#[tauri::command]
fn restore_project(project_id: i64, state: State<'_, AppState>) -> Result<Project, String> {
    database(&state)?.restore_project(project_id, now_seconds())
}
