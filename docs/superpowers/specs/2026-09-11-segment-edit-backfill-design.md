...
- **编辑会把起止时间对齐到分钟。** 表单用 `datetime-local`，粒度是分钟；而原始记录来自 `now_seconds()`，是秒级。因此对一条已有段只改项目、时间原样保存，也会把边界对齐到整分钟，最多漂移 59 秒。前端刻意保持分钟粒度（未使用 `step="1"`），因为本应用的展示粒度同样是分钟——这是有意的取舍，不是缺陷。
- **前端即时校验接受带秒的 `HH:MM:SS`，后端解析只接受 `HH:MM`。** 这个不对称是刻意的：它保证「带秒的值」不会让保存按钮永久禁用。该输入经界面不可达（原生选择器在默认 `step` 下只产出分钟值）；万一出现，后端会以 `segment_datetime_invalid` 拒绝，而不是静默写入。
- **夏令时切换当天，补录默认值按秒数回推，可能与挂钟直觉不符。** 影响面是每年两天、且仅在当天早于 10:00 的默认值路径；默认值可被用户覆盖，后端以绝对秒存储，正确性不受影响。与 [CLAUDE.md](CLAUDE.md) 已声明的「夏令时切换的历史日期尚未完全适配」属同一类问题。
- **计时进行中补录会占用一段时间。** 进行中的段被视为延伸到未来，因此晚于它起点的补录会被判为重叠（提示「该时段与已有记录重叠」）。这是数据模型正确的表现——否则同一段时间会被双计——但提示文案不含「进行中的计时」这层信息，用户需自行对照明细行上的「计时中」。
